...
class Simulator:
    def __init__(self, model, controller, dt=0.005, t_max=10, theta_limit=np.pi/4):
        self.model = model
    ...
    def run(self, initial_conditions, output_file):
        ...
        t = 0
        while t < self.t_max:
            V = self.controller.compute(states[-1][2], states[-1][3], self.dt)  # Get control voltage
            sol = solve_ivp(self.model.equations_of_motion, [t, t + self.dt], states[-1], 
                            args=(self.controller,), method='RK45', t_eval=[t + self.dt])
            new_state = sol.y[:, -1]
